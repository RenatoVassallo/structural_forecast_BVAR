...
path = [306; 328; 327; 261; 188];%ones(12,1).*160; 
...
yhat1(1:L,:)=Y_all(end-L+1:end,:); % Start the forecast using March 2023 as the latest data point
for i=L+1:horizon+L   
    x=[];
    for j=1:L
    x=[x yhat1(i-j,:)];
...
yhat2(1:L,:)=Y_all(end-L+1:end,:); % Start the forecast using March 2023 as the latest data point
for i=L+1:horizon+L   
    x=[];
    for j=1:L
    x=[x yhat2(i-j,:)];
...
for igibbs=1:REPS
    ...
    datag=[d;yhatg]; %appended data without COVID-19 obs
    YSTAR=datag;
    %take lags
    XSTAR=[];
    for j=1:L
    ...
    yhatg(1:L,:)=Y_all(end-L+1:end,:); % Start the forecast using March 2023 as the latest data point
    for i=L+1:horizon+L   
        x=[];
        for j=1:L
        x=[x yhatg(i-j,:)];
...
% FIGURE 1: UNCONDITIONAL AND STRUCTURAL FORECAST
%--------------------------------------------------
...
figure('Name','Unconditional and Structural Forecast','NumberTitle','off')
...
TT = datetime(2021,06,01):calmonths(1):datetime(2023,08,01);
percentiles = [10 20 30 40 50 60 70 80 90];
fan_color_con = [1 0.39 0.28];
fan_color_unc = [0.39 0.2 1];

% Compute the percentiles for each matrix in 'outcon' and 'outunc'
pct_values_outcon = cell(1, size(outcon, 3));
pct_values_outunc = cell(1, size(outunc, 3));
for i = 1:size(outcon, 3)
    pct_values_outcon{i} = prctile(outcon(:, end-size(TT,2)+1:end, i), percentiles);
...
for i = 1:size(outcon, 3)
    ...
    h_outunc = plot(TT, median(outunc(:, end-size(TT,2)+1:end, i)), 'Color',fan_color_unc,'LineWidth', 1.5);
    hold on;
    fill([TT, fliplr(TT)], [pct_values_outunc{i}(1,:), fliplr(pct_values_outunc{i}(end,:))], fan_color_unc, 'EdgeColor', 'none', 'FaceAlpha', 0.15);
    for j = 2:numel(percentiles)-1
        fill([TT, fliplr(TT)], [pct_values_outunc{i}(j,:), fliplr(pct_values_outunc{i}(end-j+1,:))], fan_color_unc, 'EdgeColor', 'none', 'FaceAlpha', 0.075);
    end

    % Plot the fan chart for 'outcon'
    h_outcon = plot(TT, median(outcon(:, end-size(TT,2)+1:end, i)), 'Color',fan_color_con, 'LineWidth', 1.5);
    fill([TT, fliplr(TT)], [pct_values_outcon{i}(1,:), fliplr(pct_values_outcon{i}(end,:))], fan_color_con, 'EdgeColor', 'none', 'FaceAlpha', 0.15);
    for j = 2:numel(percentiles)-1
        fill([TT, fliplr(TT)], [pct_values_outcon{i}(j,:), fliplr(pct_values_outcon{i}(end-j+1,:))], fan_color_con, 'EdgeColor', 'none', 'FaceAlpha', 0.075);
    end
    ...
    forecast_end = datetime(2023, 08, 01);
...
leg = legend([h_outcon, h_outunc], {'Structural Forecast (Median)', 'Unconditional Forecast (Median)'},'Orientation', 'horizontal','FontSize', 12);
leg.Layout.Tile = 'south';
set(gcf,'position',[x0,y0,width,height])
exportgraphics(t, 'structural_forecast.png', 'BackgroundColor','white','Resolution', 300);
